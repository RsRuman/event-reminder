...
@section('title')
    Edit Event | Event Reminder
@endsection

@section('content')
    <div class="mx-auto rounded-md overflow-hidden shadow-md">
        <div class="px-12 py-4">
            <h2 class="text-lg font-semibold text-gray-800 mb-2">Edit Event</h2>
            <form action="{{ route('events.update', request('id')) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label for="title" class="block text-sm font-medium text-gray-700">Title</label>
                    <input id="title" name="title" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('title', $event->title) }}">
                    @error('title')
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="4" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $event->description) }}</textarea>
                    @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="datetime-local" id="date" name="date"
                           ...
                           min="{{ date('Y-m-d', strtotime($event->date)) }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const initialRecipients = @json($event->recipients);
            ...
            initialRecipients.forEach(email => tagify.addTags(email));
        });
    </script>
@endsection
